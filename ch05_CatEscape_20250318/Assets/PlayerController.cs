...
{
    // 멤버 변수 선언
    float fMaxPositionX = 10.0f; //플레이어가 좌, 우 이동시 게임창을 벗어나지 않도록 Vector 최대값 설정 변수 
    float fMinPositionX = -10.0f; //플레이어가 좌, 우 이동시 게임창을 벗어나지 않도록 Vector 최소값 설정 변수 
    float fPositionX = 0.0f;    //플레이어가 좌, 우 이동할 수 있는 X 좌표 저장 변수 


    /*
     * Start 메소드
     *  미리 정의된 특수 함수(메소드)로써, 이 특수 함수들을 C#에서는 함수를 메소드라고 함
     *  MonoBehaviour 클래스가 초기화 될 때 호출 되는 이벤트 함수 
     *  프로그램이 시작할 때 한 번만 호출이 되는 함수로 보통 컴포넌트를 받아오거나 업데이트나 다른 함수에서 사용하기 초기화 해주는 기능
    ...
    void Start()
    {
         ...
         * 스마트폰은 60, 고속의 PC는 300이 될 수 있는 디바이스 성능에 따라 게임 동작에 영행을 미칠 수 있음
         * 프레임레이트를 60으로 고정
         */
    ...
    {
        /*
         * 키가 눌렸는지 검출하기 위해서는 Input 클래스의 GetKeyDown 메서드를 사용함
         * 이 메서는 매개변수로 전달한 키가 눌리는 순간 true를 한 번 반환
         * GetKeyDown 메서드는 지금까지 사용하던 GetMouseButtonDown 메서드와 비슷함으로 쉽게
         *
         */
        ...
        {
            // 왼쪽으로 이동하기 전에 경계를 벗어나지 않도록 확인
                transform.Translate(-2, 0, 0);  // 왼쪽으로 '3'만큼 이동
        }

        // 오른쪽 화살표가 눌렸을 때-> GetKeyDown(KeyCode.RightArrow)
        if (Input.GetKeyDown(KeyCode.RightArrow))
        {
            // 오른쪽으로 이동하기 전에 경계를 벗어나지 않도록 확인
                transform.Translate(2, 0, 0);   // 오른쪽으로 '3'만큼 이동
        }
