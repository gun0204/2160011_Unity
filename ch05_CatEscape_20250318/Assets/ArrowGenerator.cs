 ...
 * - 프레임과 플레임 사이의 시간 차이를 대나무 통(delta변수)에 모으고 (합계) 1초 이상이 되면 대나무 통을 비움
 * - 대나무 통을 비우는 시점인 1초에 한 번씩 화살이 생성됨
 * - Instantiate 메서드
 *  -- 게임을 실행하는 도중에 게임오브젝트를 생성할 수 있음
 *  -- 화살 프리팹을 이용하여, 화살 인스턴스를 생성하는 메서드
 * - Random.Range 메서드 : 램덤 값을 쉽게 생성할 수 있는 방법
 *  -- Random 클래스는 흔히 요구되는 다양한 타입의 랜덤 값을 쉽게 생성 할 수 있는 방법을 제공 
 *  -- 사용자가 제공한 최솟값과 최대값 사이의 임의의 숫자를 제공함
 *      -- 첫 번째 매개변수보다 크거나 같고, 두 번째 매개변수보다 작은 범위에서 무작위 수를 램덤하게 반환
 */
...
{
    /*
     * 제네레이터 스크립트에 프리팹 전달 방법
     *  - arrowPrefab 변수에 프리팹 실체를 대입하기 위해서 public 접근 수식자
     *  - 멤버변수 선언시 public으로 선언하면 Inspecter 창에서 Prefab 설계도 대입할 수 있도록 보임
     *  - 화살 대량 생산을 위해서 양산기계(제네레이터스크립트)에 넘겨 줄 Prefab 설계도를 넘겨주어야함.
     */

    public GameObject gArrowPrefab = null; // 화살 Prefab을 넣을 빈오브젝트 상자 선언(중요)
