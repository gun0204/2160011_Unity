...
public class PlayerController2 : MonoBehaviour
{
    ...
    // 플레이어 좌우 움직이는 속도
    float fPlayer1MoveSpeed = 0.2f;
    // 멤버 변수 선언
    float f1MaxPositionX = 2.0f; //플레이어가 좌, 우 이동시 게임창을 벗어나지 않도록 Vector 최대값 설정 변수 
    float f1MinPositionX = -2.0f; //플레이어가 좌, 우 이동시 게임창을 벗어나지 않도록 Vector 최소값 설정 변수 
    float f1PositionX = 0.0f;    //플레이어가 좌, 우 이동할 수 있는 X 좌표 저장 변수 
    ...
    void Start()
    {
        ...
        this.ScoreText = GameObject.Find("Score2");
    }
    ...
    public void AddScore(int value) //  점수 값 인자로 받음
    {
        ...
        scoreText.text = "Score2: " + gScore.ToString("0");
    }
    ...
    void Update()
    {
         ...
         */
        fPlayer1MoveSpeed = Mathf.Abs(m_rigid2DCat.linearVelocity.x);
        if (fPlayer1MoveSpeed < fMaxWalkSpeed)
        {
        ...
        {
            m_animatorCat.speed = fPlayer1MoveSpeed / 2.0f;
        }
        else
        {
       ...
       */
        f1PositionX = Mathf.Clamp(transform.position.x, f1MinPositionX, f1MaxPositionX);
        transform.position = new Vector3(f1PositionX, transform.position.y, transform.position.z);
